...
export default function AdminShell({children}:{children:React.ReactNode}){
 ...
 return <div className="admin-layout"><aside className="admin-sidebar"><a className="admin-logo" href="/panel"><span>ST</span><div><b>SACA UN TURNITO</b><small>Panel de gestion</small></div></a><nav>{nav.map(([href,icon,label])=><a key={href} href={href} className={path===href?'active':''}><span>{icon}</span>{label}</a>)}</nav><div className="sidebar-bottom"><a href="/reservar" target="_blank">AB <b>Abrir reservas</b></a><button onClick={exit}>SA <b>Cerrar sesion</b></button></div></aside><div className="admin-content"><header className="mobile-admin-bar"><a href="/panel">SACA UN TURNITO</a><a href="/reservar">Reservar</a></header>{children}</div></div>
}
